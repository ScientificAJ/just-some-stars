...
| JSS-016 | The final Task 12 Android video is presentation-stability evidence only: user review found no visible movement, Jump, Interact or Lens transition. The same paid instance expired before final-APK cadence, memory, background/resume, root-Back and logcat could be repeated after the Lens-only input rebinding; remote Vulkan overdraw was also not attributable. | Next declared paid-device/performance boundary, no later than Task 30 | None; movement has a separate pixel diff, behavior has focused PlayMode coverage, Lens has exact RED/GREEN and before/after proof, and the user approved the final visual result |
