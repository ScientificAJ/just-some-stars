...
```mermaid
flowchart LR
    Input[Touch + accessibility-adjusted input] --> Gameplay[Flight / exploration / Discovery Lens]
    Gameplay --> Mission[Mission director]
    Mission --> State[Shared game state]
    State --> Atlas[Cosmic Atlas]
    State --> Save[Versioned local save]
    Purchases[RevenueCat purchase adapter] --> Cosmetics[Cosmetic entitlements]
    Cosmetics --> State
    Content[Data-driven destination content] --> Mission
```
